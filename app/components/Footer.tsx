...
export default function Footer() {
  const [timeLeft, setTimeLeft] = useState({ days: 18, hours: 0, minutes: 5 });

  useEffect(() => {
    // Basic countdown simulation for the visual effect
    ...
    <footer className="relative w-full h-[600px] overflow-hidden bg-black flex flex-col justify-between py-10 px-6 md:px-12 text-white border-t border-white/5">
      ...
      <div className="relative z-10 flex flex-col-reverse sm:flex-row justify-between items-center sm:items-end w-full max-w-7xl mx-auto gap-6 sm:gap-0">
        ...
        <div className="flex items-center gap-4">
          <a href="#" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/20 transition-colors text-white/70 hover:text-white">
        ...
        <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-full px-5 py-2.5 backdrop-blur-md sm:w-[200px] justify-end">
          ...
          <span className="font-mono font-bold text-xs text-white/90 tracking-wide">
            {timeLeft.days}d {timeLeft.hours.toString().padStart(2, '0')}h {timeLeft.minutes.toString().padStart(2, '0')}m <span className="text-white/40 font-normal">left to apply</span>
          </span>
        </div>
      </div>
    </footer>
